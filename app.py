...
# --- 0. 系統設定 ---
st.set_page_config(page_title="AI 實戰戰情室 V13.20 (細節標註修復版)", layout="wide", page_icon="🦅")
...
# --- 1. 自選股儲存系統 ---
DEFAULT_LIST = ['NVDA', 'TSM', 'AAPL', 'MU', 'TSLA', 'ONDS', 'QQQ', '0050.TW', '2330.TW', '3535.TW']
...
def save_anchor(ls, ticker, news_title, score):
    try:
        ...
        data = {"title": news_title, "score": score, "date": datetime.now().strftime("%Y-%m-%d")}
        ls.setItem(key, data)
    except: pass
...
def get_chinese_ticker_name(ticker):
    mapping = {'NVDA': '輝達', 'TSLA': '特斯拉', 'AAPL': '蘋果', 'AMD': '超微', 'MSFT': '微軟', 'GOOG': '谷歌', 'AMZN': '亞馬遜', 'META': '臉書', 'TSM': '台積電', 'MU': '美光', 'INTC': '英特爾', 'ONDS': 'ONDS'}
    base = ticker.split('.')[0]
...
def fetch_strict_news(ticker, is_macro=False):
    try:
        ...
        if is_macro:
            ...
            if resp.status_code == 200:
                ...
                for item in root.findall('.//item')[:6]:
                    items.append(parse_rss_item(item, "macro"))
            return items
        ...
        if is_tw_stock:
            ...
            for url, cat in [(url_sec, "us_sec"), (url_news, "us_news")]:
                try:
                    resp = requests.get(url, headers=headers, timeout=4)
                    if resp.status_code == 200:
                        root = ET.fromstring(resp.content)
                        limit = 4 if cat == "us_sec" else 6
                        for item in root.findall('.//item')[:limit]:
                            items.append(parse_rss_item(item, cat))
                except: pass
...
def parse_rss_item(item, category):
...
def analyze_news_weight_strict(title, category):
    ...
    if any(w in t for w in farm_words): return 0, "", False 

    trap_words = ['減持', '賣出', '劣於', '降評', '損', '疲軟', '警告', '重挫', '砍單', '大跌', '崩盤', '利空', 'sell', 'underweight', 'miss', 'probe', 'lawsuit']
    for w in trap_words:
        if w in t: return -2.0, '<span class="tag-red">Risk</span>', False
    ...
    if any(x in t for x in ['營收', 'revenue', 'eps', 'profit', '獲利', '自結', 'free cash flow', '財報', 'earnings', '8-k', '10-q', '重大訊息', '重訊']):
    ...
    elif category == "us_sec" or 'form 4' in t or '申報轉讓' in t:
        vips = ['ceo', 'cfo', 'musk', 'jensen', 'cook', '黃仁勳', '馬斯克', '張忠謀']
        is_vip = any(v in t for v in vips)
        if any(x in t for x in ['buy', 'purchase', 'bought', '買進', '增持']):
            if is_vip: score = 4.0; tag = '<span class="tag-vip">👑 VIP買進</span>'; is_major = True
            else: score = 1.5; tag = '<span class="tag-sec">📝 內部人買進</span>' 
        elif any(x in t for x in ['sell', 'sold', 'dispose', '賣出', '減持']):
            if is_vip: score = -3.0; tag = '<span class="tag-red">⚠️ VIP拋售</span>'
            else: score = -1.0; tag = '<span class="tag-gray">內部人賣出</span>' 
        ...
        action_verbs = ['說', '稱', '表示', '回應', '宣布', 'talks', 'says', 'confirms', 'announces', '指出', '強調']
        if any(v in t for v in action_verbs): score = 2.0; tag = '<span class="tag-vip">👑 VIP發言</span>'
        else: score = 0.5; tag = '<span class="tag-gray">🗣️ VIP相關</span>'

    if not tag:
        if score > 0: tag = '<span class="tag-hard">📈 利多</span>'
        elif score < 0: tag = '<span class="tag-red">📉 利空</span>'
        
    return score, tag, is_major

def get_macro_environment():
    news = fetch_strict_news("Macro", is_macro=True)
    score = 0
    for n in news:
        t = n['title'].lower()
        if any(w in t for w in ['hike', 'inflation', '升息', '通膨', '鷹']): score -= 1.5
        if any(w in t for w in ['cut', 'pause', '降息', '鴿']): score += 1
    
    if score <= -3: return "Fed 偏鷹", "系統風險高", "sig-red", -2
    elif score < 0: return "宏觀偏空", "震盪觀望", "sig-orange", -1
    else: return "宏觀穩健", "多頭支撐", "sig-green", 0
...
def detect_smart_money_divergence(df):
    ...
    if price_drop and ad_rise and rsi < 50: return "🎯 主力背離吸籌"
    if rsi < 30 and df['Volume'].iloc[-1] > df['Vol_SMA5'].iloc[-1]: return "⚡ 恐慌殺盤"
    return None
...
def predict_target_and_rating(df):
    ...
    target_long = max(recent_high * 1.15, target_short * 1.1)
...
st.title(f"📈 {current_ticker} 實戰戰情室 V13.20")
...
try:
    ...
    with st.spinner(f"🕵️‍♂️ 啟動{engine_name}：正在掃描並過濾農場新聞..."):
        items = fetch_strict_news(current_ticker, is_macro=False)
    ...
    anchor = get_valid_anchor(ls, current_ticker)
    if anchor: news_score += anchor['score']
    
    for item in items:
        s, tag, major = analyze_news_weight_strict(item['title'], item['cat'])
        ...
        if major and s > 0: save_anchor(ls, current_ticker, item['title'], 3.0)
        processed.append({'data': item, 'score': s, 'tag': tag})

    if has_major:
        verdict = f"🚀 強力看漲 (霸體)！重訊/財報利多 (+{news_score:.1f})"
        v_col = "#4ade80"
        ...
        if macro_score < 0: m_disp = f'<div class="macro-alert">{macro_txt}：市場風險高，評分已下修</div>'
        else: m_disp = f'<div style="color:orange;">{macro_txt}</div>'
        
        if news_score >= 3: verdict = "📈 偏多操作 (基本面支撐)"; v_col = "#ffc107"
        elif news_score <= -2: verdict = "📉 偏空看待 (利空罩頂)"; v_col = "#ff6b6b"
        else: verdict = "☁️ 觀望整理 (缺乏驅動力)"; v_col = "gray"

    nc1, nc2 = st.columns([0.4, 0.6])
    with nc1:
        st.markdown(m_disp, unsafe_allow_html=True)
        if anchor:
            st.markdown(f"""<div class="anchor-box"><b>⚓ 記憶錨定</b><br>{anchor['date']} {anchor['title']}</div>""", unsafe_allow_html=True)
            
        st.markdown(f"""
        <div class="ai-box" style="text-align:left;">
            <h3 style="color:white; margin:0;">🔮 AI 戰情推演</h3>
            <div style="font-size:18px; color:{v_col}; font-weight:bold; margin-top:10px;">{verdict}</div>
            ...
            for p in processed:
                ...
                <div class="news-card" style="{style}">
                    <a href="{p['data']['link']}" target="_blank" class="news-link">
                        {p['tag']} <b>{p['data']['date']}</b> | {p['data']['title']}
                    </a>
                </div>
                """, unsafe_allow_html=True)
        else:
