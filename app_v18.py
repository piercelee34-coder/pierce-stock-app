...
def get_stock_name(ticker):
    ...
    if ".TW" in ticker or ".TWO" in ticker:
        ...
        for k in keys_to_delete: 
            if k in local_map: del local_map[k]
...
def get_tw_all_macro():
    ...
    api_token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VyX2lkIjoia2Fpc2xpcGtub3QzNCIsImVtYWlsIjoicGllcmNlLmxlZS4zNEBnbWFpbC5jb20ifQ.0VqcmGdadUXGR7mQzrStPewfBRHsM1_Fi9fikea1lCI"
    url = "https://api.finmindtrade.com/api/v4/data"
    headers = {'User-Agent': 'Mozilla/5.0'}
    
    results = {"success": False, "oi_df": pd.DataFrame(), "retail_ratio": None, "current_tx_oi": None, "pc_ratio": None, "spot_df": pd.DataFrame(), "msg": ""}
    err_msgs = []
    
    def fetch(dataset, data_id=None):
        params = {"dataset": dataset, "start_date": start_date, "token": api_token}
        ...
        except Exception as e: err_msgs.append(f"{dataset}: {str(e)[:15]}")
        return pd.DataFrame()
    ...
    def get_net_oi(df_subset):
        ...
        if net_col: return net_col
        lc = find_col(df_subset, ['long_open_interest', '多方未平仓'])
        sc = find_col(df_subset, ['short_open_interest', '空方未平仓'])
        if lc and sc:
            df_subset['temp_net'] = pd.to_numeric(df_subset[lc], errors='coerce') - pd.to_numeric(df_subset[sc], errors='coerce')
            return 'temp_net'
        return None
            ...
            if not df_tot.empty:
                ...
                if dc_tot and oc_tot:
                    ...
                    if not tot_row.empty:
                        ...
                        if not pd.isna(val) and val > 0: total_oi = val
    ...
    if not df_spot.empty:
        ...
        if name_col_s:
            ...
            if date_col_s:
                ...
                if net_col_s: df_spot_f['Net_Buy'] = pd.to_numeric(df_spot_f[net_col_s], errors='coerce') / 1e8
                elif buy_col and sell_col: df_spot_f['Net_Buy'] = (pd.to_numeric(df_spot_f[buy_col], errors='coerce') - pd.to_numeric(df_spot_f[sell_col], errors='coerce')) / 1e8
                else: df_spot_f['Net_Buy'] = 0
                results["spot_df"] = df_spot_f[['Date', 'Net_Buy']].dropna().groupby('Date').sum().reset_index().sort_values('Date').tail(120)
...
def generate_projection_points(df, trend_text, cur_p, iron_p, is_brk, ph_support, ph_resist):
    ...
    td_sell_cur = df['TD_Sell_Seq'].iloc[-1]; recent_high_20 = df['High'].tail(20).max()
...
def analyze_strategic_signals(df):
    ...
    rsi = latest['RSI']
    rsi_text, rsi_color = (f"{rsi:.0f} 過熱", "sig-red") if rsi > 70 else (f"{rsi:.0f} 超賣", "sig-green") if rsi < 30 else (f"{rsi:.0f}", "sig-gray")
...
def get_earnings_status(ticker):
    ...
    next_date = "N/A"; last_result = "⚪ 無前季數據"
    try:
        t = yf.Ticker(ticker); hist = None
        ...
        if hist is not None and not hist.empty:
            ...
            if not future_dates.empty: next_date = future_dates.index.min().strftime('%Y-%m-%d')
            try:
                past_dates = hist[hist['Reported EPS'].notna()]
                if not past_dates.empty:
                    last = past_dates.iloc[0]
                    act = last.get('Reported EPS', np.nan); est = last.get('EPS Estimate', np.nan)
        ...
        if next_date == "N/A":
            try:
                cal = t.calendar
                if isinstance(cal, dict) and 'Earnings Date' in cal and len(cal['Earnings Date']) > 0: next_date = cal['Earnings Date'][0].strftime('%Y-%m-%d')
                elif isinstance(cal, pd.DataFrame) and 'Earnings Date' in cal.columns and not cal.empty: next_date = pd.to_datetime(cal['Earnings Date'].iloc[0]).strftime('%Y-%m-%d')
            except: pass
...
def get_tactical_advice(df, cur_p, t_s, iron_p, ph_support, ph_resist, bias_ma5, bias_limit, engine_type):
    ...
    if cur_p > b_upper * 1.02: return "🔥 軌道噴發 (高度警戒)", f"目前股價已飛出布林上軌，雖然動能極強，但隨時面臨暴力回檔均線的風險。嚴禁追高！", "#ef4444"
    if bias_ma5 >= bias_limit * 0.8: return "⚠️ 短線超買 (乖離偏高)", f"MA5 正乖離達 {bias_ma5:.1f}%，已逼近該股性極限。短線追高動能耗盡，請勿追高！", "#f97316"
    ...
    if 1 <= td_s <= 7 and cur_p > ma20: return "🚀 主升歡樂帶 (抱緊處理)", f"目前上漲動能強勁 (TD {int(td_s)})，且穩居月線之上。正處於上升通道，請【沿著均線續抱】，享受獲利，不輕易下車！", "#22c55e"
    if td_b >= 8 or rsi < 30: return "💎 空頭力竭 (準備破底翻)", f"殺盤動能即將耗盡 (目前 TD 下跌 {int(td_b)} 或是 RSI超賣)，股價已進入絕佳的潛在支撐區。請密切關注反轉向上的【右側買點】！", "#facc15"
    if 1 <= td_b <= 7 and cur_p < ma20: return "🔪 恐慌殺盤中 (嚴禁接刀)", f"目前處於主跌段 (TD 下跌 {int(td_b)})，賣壓極度沉重。在底部紅字 8 或 9 出現之前，請綁好雙手【絕對不要進場買進】！", "#f97316"
    if cur_p > ma20: return "📈 多頭延續 (沿均線續抱)", "目前股價穩居月線 (MA20) 之上，多頭格局不變。建議沿均線續抱。", "#22c55e"
    else: return "⚖️ 震盪整理 (耐心觀望)", "目前股價處於區間震盪，方向尚未明確。建議耐心觀望，等待突破或測底訊號。", "#9ca3af"
...
if 'SMA_5' in p_data.columns:
    fig.add_trace(go.Scatter(x=p_data.index, y=p_data['SMA_5'], mode='lines', line=dict(color='#f97316', width=1.5), name='MA5 (周線)'), row=1, col=1)
if 'SMA_10' in p_data.columns:
    fig.add_trace(go.Scatter(x=p_data.index, y=p_data['SMA_10'], mode='lines', line=dict(color='#38bdf8', width=1.5), name='MA10 (雙周)'), row=1, col=1)
if 'SMA_20' in p_data.columns:
    fig.add_trace(go.Scatter(x=p_data.index, y=p_data['SMA_20'], mode='lines', line=dict(color='#d946ef', width=2), name='MA20 (月線)'), row=1, col=1)
if 'SMA_60' in p_data.columns:
    fig.add_trace(go.Scatter(x=p_data.index, y=p_data['SMA_60'], mode='lines', line=dict(color='#2563eb', width=2), name='MA60 (季線)'), row=1, col=1)
...
if is_tw:
    ...
    def get_retail_stage(ratio):
        if ratio is None: return "資料讀取中", "#666"
        if ratio >= 25: return "💥 極度瘋狂 (大崩盤警戒)", "#dc2626"
        elif ratio >= 10: return "⚠️ 過度樂觀 (反向偏空)", "#f97316"
        elif ratio > -10: return "⚖️ 多空膠著 (無明顯方向)", "#9ca3af"
        elif ratio > -25: return "🍀 過度悲觀 (反向偏多)", "#84cc16"
        else: return "🚀 極度恐慌 (絕佳軋空底)", "#22c55e"

    m_data = get_tw_all_macro()
    retail_ratio = m_data.get("retail_ratio")
    pc_ratio = m_data.get("pc_ratio")
    ...
    if err_msg: st.warning(f"⚠️ API 狀態回報: {err_msg}")
    ...
    st.header("🏛️ 美股專屬：總經情緒雙核觀測站 (示範數據)")
    bc1, bc2 = st.columns([0.6, 0.4])
    naaim_df = get_naaim_data()
    aaii_bull, aaii_neu, aaii_bear = get_aaii_data()
    
    with bc1:
        st.markdown('<div class="ai-box" style="text-align:left;"><h4 style="color:white; margin:0; margin-bottom:10px;">📈 NAAIM 主動經理人曝險指數</h4>', unsafe_allow_html=True)
        fig_naaim = go.Figure()
        fig_naaim.add_trace(go.Scatter(x=naaim_df['Date'], y=naaim_df['Exposure'], fill='tozeroy', mode='lines+markers', line=dict(color='#38bdf8', width=2), marker=dict(size=6, color='#38bdf8')))
    ...
    with bc2:
        aaii_html = (
            f'<div class="ai-box" style="text-align:left; height:100%;">'
            f'<h4 style="color:white; margin:0; margin-bottom:15px;">🧠 AAII 散戶情緒調查</h4>'
